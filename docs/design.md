...
| I2 频率统计 | rowid → count | 与 rowid 对齐的平铺 u32 数组（构建期一次写） | 分层：热集选边、缓存保留策略、指标；训练语料 Zipf 有效 |
| I3 段式列表 | (segment_id, key) | 段内：排序索引 + 连续值块 + 统计头（min/max/count） | Store-P 训练流：阶段化顺序扫描 + 段缓存；跨段只读一次 |

I2 之所以有：语料侧 Zipf 严格成立（论文与词频统计）；聊天侧失效（实测 1.3% 驻留），
所以**频率索引用于"训练预处理"，绝不作为"推理正确性"的依赖**——这正是本轮讨论最大的认知修正。
...
- 吞吐模型：
  - 每 token 独特行（Zipf 强）：语料侧重复率高 → 段窗口内 T1 命中率很高，跨段接近顺序读；
...
3. **频率索引 + 分层**：引擎只做"放哪一层"，不做"为访问分布服务"（语料侧 Zipf；聊天侧我们已证明热集无益，文档明示）；
...
### 7.1 量化目标（M0 数据修正前为设计预期）

| 指标 | 符号 | 目标 |
|---|---|---|
| 冷点查（1 batch = 16-32 行；NVMe 冷） | p50 / p95 | ≤ 2ms / ≤ 4ms（一次 badge 读量级）；交叠后对 decode 的尾差 ≤0.2ms |
| 热查（T1 命中） | p50 | ≤ 50µs；批 4K 行 < 2ms |
| 训练流有效吞吐 | 有效 tok/s | ≥ 100K tok/s（受 CPU hash/tokenize 上限影响，M0 P3 实测） |
| 字节放大率 | u02 | ≤ 2×（目标：Store-P ≈ 1.0×；Store-I 原始 ≤ 4×） |
| 磁盘增量 | — | 每表 ≤ 表大小（Store-P 视图另 +表大小，可选） |
| 端到端 decode（CPU） | tok/s | 50 tok/s（≤1B Q4 @ M 系列 / 0.5-1B @ x86）；配 MTP 后 100 tok/s 目标 |
| 端到端 decode（消费 GPU） | 与引擎基线差 | ≤ 5%（vLLM mmap 组对标） |
| 内存 | RAM 占用 | 固定于 `--ram-budget`，不随表大小增长（除显式热集） |

### 7.2 指标规约（探针开始即收集，报告常包）

- u00 tier hit-rate（T1/T2/T3 各自命中比）；
- u01 单次 fetch 延迟分布（p50/p95/p99）；
- u02 **字节放大率** = （实际读字节）/（最小需要字节）——社区最反面的教科书数字：
  llama.cpp / SGLang 均报告过"138KB 读/2.5KB 用"；我们以 ≤2x 为验收红线；
- u03 有效 tok/s（查询路径单独统计）；
- u04 磁盘占用/预取窗口命中情况；
- 报表：`probe-report.md` + CSV 源数据。
