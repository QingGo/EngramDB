...
### 7.3 服务预算：把"≤5%"反推成每 token 绝对开销（2026-09-12 定稿）

**动机**：§7.1 的"端到端 decode 对引擎差 ≤5%"在现有硬件上**不可测**——它需要能放下
Qwen3.8-Flash-Next（125B 主干）或 V4.1-Flash（552B 主干）的机器。一个无法测量的验收项会
永久挂在 ⏳ 上，并让所有其他工作都可以自称"在为它做准备"（roadmap §29.2）。

**做法**：把百分比反推成**绝对预算**，在本机即可测量；绝对预算成立时，百分比在算术上自动成立。

**推导**（假设与出处均列出）：

| 步骤 | 取值 | 出处 |
|---|---|---|
| 端到端 decode 目标 | ≥50 tok/s，配 MTP 冲 100 tok/s | §7.1 目标表 |
| 每 token 时间 | 20 ms（50 tok/s）～ 10 ms（100 tok/s） | 由上行推出 |
| ≤5% 预算 | **1000 μs ～ 500 μs** | 5% × 上行 |
| **门禁取值** | **500 μs/token（保守端）** | 取 100 tok/s 侧，留最大余量 |
| 分解到行（Qwen PLE，16 行/token） | 31 μs/行 | 500 / 16 |
| 分解到行（V4.1 Engram，48 行/token） | 10 μs/行 | 500 / 48 |

**适用口径**：该预算覆盖**存储读取 + 反量化 + 引擎侧 adapter** 的全部 Engram 相关开销，
不含注意力/MoE 等主干计算。门禁实现见 `scripts/real_perf_gate.py` 的 `MAX_US_PER_TOKEN = 500.0`。

**实测对照**（2026-09-12；真表 128 shard；**warm-up 2 次后取 5 次中位数**；
rowid **全表随机**；见 `probes/serving_ab_v041.json`）：

| 路径 | tok/s | μs/token | 对 500 μs 预算 |
|---|---|---|---|
| `Store.fetch` | 92,000–105,132 | 9.51–10.87 | ⚠️ **warm 口径**（见下） |
| `PleMemory.fetch_raw` | 76,273–84,670 | 11.81–13.11 | ⚠️ **warm 口径** |
| `PleMemoryAdapter`（torch） | 48,176–48,621 | 20.57–20.76 | ⚠️ **warm 口径** |

> 🛑 **上表是 warm（页缓存命中）口径，不能用来支持"有 24× 余量"**。
> 该结论已在 `roadmap.md` §30.1 被自己的探针推翻，并在 §31.4 用原生 NVMe + 真实 PLE 数据
> 定案。**冷读实测**（自校验强制，`fadvise` 冷、每行 1 个独立 4 KiB 页）：
>
> | 形态 | @8 线程（当前默认） | @16 | @32 |
> |---|---|---|---|
> | Qwen 16 行/token | **204 μs（41%）** | 119 μs（24%） | 76 μs（15%） |
> | V4.1 48 行/token | **604 μs（121%，超预算）** | 353 μs（71%） | 223 μs（45%） |
>
> warm 表仍然有用——它证明**引擎侧代码路径本身**只花 ~10–21 μs/token，
> 即开销几乎全在介质上；但它**不是**预算结论。

**修正声明（推翻此前结论）**：README §3.3 记录的"真表 `PleMemoryAdapter` 约 1.6K–2.0K tok/s"
**不可复现**，是两处方法学缺陷叠加的产物：

1. **单次、无 warm-up 测量**：外盘首次触达的页缓存缺失主导了读数。同一份未改动的代码，
   冷测 1.7K tok/s、热测 25.8K tok/s（**15× 离散**）。
2. **rowid 分布失真**：`flat = [i % rows_available ...]` 只访问 rowid 0..65535，
   即 51.2 GB 表中的 **10.5 MB 窗口**——所谓"真表"路径实际从未触碰磁盘。

两处均已修复（`scripts/bench_serving_ab.py`：`--warmup/--reps/--rowids`），
门禁改为**读取 adapter 指标，"未测量" = FAIL**。
