...
# §42 第三十二轮系统性思考（Session 45–47 之后）

### 42.1 终极目标：补上第二个判据，并把「能服务」和「免费」分开

§38.1 把目标收紧为「**没有任何一层内存装得下、且硬件不支持内核直接解引用时仍可服务**」。
Session 45–47 之后，这个句子里**两个半句的地位不一样了**：

| 半句 | 状态 | 证据 |
|---|---|---|
| **能跑**（容量） | ✅ **达成** | 真实 51.2 GB 表 + 非 HMM 显卡 + graph 模式；三条独立正确性证据（引擎 hash 逐位一致 / `Store.fetch` 与独立 `pread` 字节相等 / 每次 replay 5/5） |
| **能服务**（开销） | ❌ **未达成** | 读**完全暴露**：warm 196.5–1094.9 µs、cold 406.5–10603.8 µs，全部落在关键路径上 |

更要紧的是，本轮把「能服务」的判据本身说清楚了 —— **之前把它和「免费」混为一谈了**：

> `t_read ≤ τ(1)` 只是「**免费**」的判据（读能被前一层完整盖住）。
> **「能服务」的判据是另一个**：`t_read` 相对于**模型自己的单步时间**要足够小。
> 藏不住 ≠ 不能用。

这个区分是新的，而且它是可判定的：
- 2048 行 1094.9 µs，若模型单步是 20 ms，占比 **5.5%** —— 与项目最早那条「A/B ≤5%」的验收线同一量级，**接近达标**；
- 同一个数若相对 τ(1)=325 µs，是 **3.4×** —— 看起来像灾难。

⇒ **「完全暴露」这个说法本身需要重新校准。** 它描述的是机制（读是串行的），
不等于后果（后果取决于分母）。本轮之前的所有表述都把机制当成了后果。

**新的可证伪判据**：在**能真跑的模型**上，graph 模式下我们的后端 vs 基线，
**端到端 tok/s 差距 ≤5%**。这条项目定义过、一次都没测过。

### 42.2 本轮技术债（V200–V207）

| 编号 | 内容 |
|---|---|
| **V200** | **交付物与记录不一致。** `0002` 让用户必须打补丁，而 §41 已证明用户**不需要**打补丁；但那个 pip 包不存在。文档现在描述了一条不存在的路径。 |
| **V201** | **「不改源码」方案未验证。** `SGLANG_EXTERNAL_MODEL_PACKAGE` + `overwrite=True` 我读了源码（registry.py 尾部两行），但**从未运行过**。这是「关于目标的断言只由源码阅读支撑」——第九条/第十条纪律的又一次实例。 |
| **V202** | **双断点重叠零实机。** §41.2 全部是设计，预期数字（142→25、196→80 µs）是从已测分量**推算**的。 |
| **V203** | **稳态工作集从未测过。** 「rowid 访问分布」从 Session 44 之前就挂在 P2 前置上，一直没做。本轮的重新表述让它变得更关键（见 42.4 Tier 0）。 |
| **V204** | **冷态对照仍不完整。** 1 TB RAM / 796 GB page cache；外科式逐页 `fadvise` 好过整表，但仍不是硬冷。真正的硬冷要 `O_DIRECT`。 |
| **V205** | **16 行的固定开销。** `Store.fetch` 在 16 行时要 ~94 µs，而 **decode 一步就是 16 行** —— 最常走的路径上有最大比例的固定成本。 |
| **V206** | **从未有过目标模型上的端到端数字。** 所有数字都是分量（µs/步），不是 tok/s。真实模型（48 层 / 51B MoE / 185 GB）本机跑不动。 |
| **V207** | **一轮 A/B 曾经在比较同一份代码。** 我的探针 `make()` 返回的是 class 而不是绑定好参数的工厂，`staging_dtype`/`device_cast` 根本没进 `__init__`，于是「fp8 vs bf16」那一轮**两臂是同一份实现**，看到的 ~70 µs 是噪声。已修并重跑。**教训不是这个 bug，而是：一个「成功」的错标签实验比一个失败的实验危险得多。** ⇒ 附加纪律见 §42.6。 |

### 42.3 借鉴矩阵：本轮新增的层

§38.3 的 L0–L7 不变，本轮在 **L6（引擎缝）** 和 **L3（发起时机）** 上有实质新增：

| 层 | 来源 | 借什么 |
|---|---|---|
| **L6 分发** | SGLang `SGLANG_EXTERNAL_MODEL_PACKAGE`（[PR #21050](https://github.com/sgl-project/sglang/pull/21050/files/) 的官方指南 + `registry.py` 尾部的 `overwrite=True`） | **不改源码、不重编的模型级注入**。注册键是**类名**，checkpoint 的 `architectures` 已是我们需要的名字 ⇒ **连 config.json 都不用改** |
| **L6 分发** | vLLM [Plugin System](https://docs.vllm.ai/en/v0.24.0/design/plugin_system/) | `[project.entry-points."vllm.general_plugins"]` —— 我们已有 `vllm_plugin.py`，只缺声明。**但只到 eager** |
| **L3 发起时机** | **反例**：[SGLang PR #29166](https://github.com/sgl-project/sglang/pull/29166) | **跨 stream 的 CUDA 预取与 capture 不共存**（`cudaErrorStreamCaptureIsolation`），上游的修法是捕获时放弃重叠改 inline。⇒ 我们的重叠设计**必须不依赖图外 stream/event**（线程池 + 纯 host I/O 满足这条） |
| **L3 发起时机** | SGLang `utils/offloader.py`（`_ModuleOffloader`） | 「提前 N 层预取 + event 同步」的 CPU 权重卸载管线 —— 与我们最像的兄弟。可借它的**结构**（提前量是个显式参数），不可借它的**同步机制**（同上） |
| **L1/L2** | FreeToken `kernel/triton/ple.py` | 表常驻 pinned + `num_warps=1`「latency-bound over PCIe, many in flight」+ **kernel 内做 dequant/scale**。第三处独立佐证「cast 留在设备」（§40.5） |

**新增第四条不冲突硬规则（§38.3 的三条之上）**：

> **分发面与机制面必须解耦。** 「外部模型包」这条路要求我们的机制**自包含** ——
> 不能 import 补丁里的 `Qwen4ExpStagedFileEmbedding` / `_adopt_embedding`。
> 于是同一个机制会有**两个宿主**（stock 树 / 打过补丁的树），`install()` 必须
> **幂等且能容忍补丁已存在**。否则两条分发渠道互相冲突。

这条不是洁癖：它直接决定了 42.4 的 Tier 2 要写多少代码（约 120 行自带实现）。

### 42.4 开发计划

排序原则：**先做那个可能让我们少做很多事的事。**

**Tier 0 · 稳态工作集（不需要 GPU，本轮唯一可能改变全局的测量）**
- 问题重述：不是「rowid 重复率是多少」，而是「**稳态工作集有多大，多久稳定下来**」。
- 为什么可能改变全局：rowid 复用 ⟺ n-gram 复用。id 对给定的 (a,b,c) 是**确定**的，
  而自然文本/代码里 3-gram 大量重复。**如果稳态工作集只有几 GB，那么内核 page cache
  就是热缓存**，我们的「cold」只是**启动瞬态**而不是稳态；冷态那一整列数字（406 µs–10.6 ms）
  就从「常态」降级为「冷启动」。
- 做法：拿真实语料跑 `engramdb.rowids_for_seq`，统计**去重后的 4 KiB 页数**随 token 数的增长曲线
  与平台期。全部 CPU，不需要 GPU。
- 判据：给出「工作集/token 数」的曲线 + 平台值。**若平台值 ≪ 宿主内存，冷态问题基本消失。**

**Tier 1 · 端到端 A/B（需要 GPU，第一次产出项目自己的验收指标）**
- 造一个**能真跑的小 Qwen4Exp checkpoint**（随机权重、真实类、真实 PLE 几何但 vocab 缩小），
  让 SGLang 真的 serve 起来，graph 模式下跑 **我们的后端 vs 基线** 的 tok/s。
- 这一步的价值不是绝对数字，而是**第一次把「分量 µs」换成项目定义的那条判据（≤5%）**。
- 顺带偿清 V206，并回答 §42.1 里那个「分母到底是多少」。

**Tier 2 · 双断点重叠（设计已完成，需实机）**
- 预期把 warm 16 行 142 → ~25 µs、单 token 单步 196.5 → ~80 µs。
- **排在 Tier 1 之后**：Tier 1 会告诉我们暴露占单步的**比例**，从而决定这条值不值得。
  这正是 P1.5 的教训 —— 不要在还没证明它重要之前优化它。

**Tier 3 · 分发（`engramdb-sglang` 包 + vLLM entry point）**
- 让用户真的能用上（偿 V200/V201）。自带约 120 行，`install()` 幂等。
- 与 Tier 2 无依赖冲突：包里的 `gather` 天然带断点。

**Tier 4 · 上游化 0001/0002 + 一条 issue（证据已齐）**
- issue 的核心不是「WILLNEED 不好」，而是「**在消费时刻发 WILLNEED 没有用**」（§39.3④），
  加我们唯一独占的 τ(L)。

**冻结**：产品广度（V4.1 落地、多表、Arrow IPC、engram-peft、C ABI 补齐）。

### 42.5 一句话

> 容量那一半**已经跑通并有独立证据**；剩下的一半被我们**自己说错了名字** ——
> 「完全暴露」是机制描述，不是后果描述。后果取决于分母，而分母（模型单步）从来没测过。
> 所以下一步不是继续优化读，而是**先量出两件事**：
> **稳态工作集有多大**（可能让冷态问题整体消失），
> 和**端到端 A/B 差多少**（项目定义过、一次都没测过的判据）。
> 在这两件事有答案之前动 Tier 2，就是第二次 P1.5。

### 42.6 附加纪律（第十一条）

> **一轮 A/B 必须在机制上自证「两臂确实不同」。**
>
> 起因 V207：探针的工厂函数返回 class 而非实例，选项没进 `__init__`，
> 于是两臂跑的是同一份代码，而实验「成功」了、还产出了一个看起来合理的差值。
> **错标签的假实验比失败的实验危险**，因为它会得到一个可用但错误的结论。
>
> 落地：①每个 A/B 打印**两臂各自的生效配置**（本轮后来打印的 `device_cast=` 就是这条的雏形）；
> ②如果两臂的差落在噪声内，**先怀疑两臂相同**，再怀疑结论是「没有差别」。
