...
> 完整版见 `docs/round-41-full-summary.md`（待补）。
...
- 机制通了之后，第一个真问题不是「多快」，而是**钱花在哪**。
  `read_static` 判决实验（行号预置、断点内零 D2H）把它定死了：
  **1.30 ms ≈ 磁盘 0.68 + D2H 0.39 + rowid 0.24**，而磁盘那一半**完全暴露**
  （`read_us_median` 461 µs 却让整步慢 675 µs）。
  ⚠️ 这里更正了本节初稿的预测：当时说「D2H 是最大项」，实测**只对了一半**。
- 修正后的四步（按实测大小排序）：① 持久线程池 + 跨 token 批量化 + pinned 暂存
  （461 → 195.9 µs，并让 `τ(L)` 真正掩盖它）② 消灭 D2H（≈0.39 ms）
  ③ rowid 移出关键路径（≈0.24 ms）④ 确认无分配器强同步。
  **②③ 可合并成一次「fill 步」**：前一步 tail 里拿 token、算行号、发起异步读。
...
**第六条：把「调研」也计入净关闭率。** 连续两轮为负就停下来做减法，不启动新方向。
